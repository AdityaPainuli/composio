...
# --agent should delegate to the setup orchestrator when a supported host is
# present, so identity and plugin installation remain one idempotent flow.
...
if ! grep -qxF 'setup --agent --target auto --yes' "$composio_log"; then
...
if grep -qxF 'login --agent --no-skill-install' "$composio_log"; then
    echo 'Expected host setup not to use the identity-only fallback.' >&2
    exit 1
fi
...
# A headless/hostless machine must still get a working agent identity instead
# of failing plugin setup because there is no host to configure yet.
rm "$bin_dir/claude"
: > "$composio_log"
